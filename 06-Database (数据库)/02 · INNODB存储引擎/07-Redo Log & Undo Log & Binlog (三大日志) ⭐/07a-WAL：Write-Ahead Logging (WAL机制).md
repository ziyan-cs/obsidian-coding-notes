...
> [!tip]- **工程要点**：WAL 是现代数据库的基石——通过将随机写转化为顺序写来大幅提升写入性能。`innodb_flush_log_at_trx_commit=1` 是唯一保证不丢数据的设置，对应核心交易场景。非核心场景可设为 2（容灾 1 秒数据丢失换取 5-10 倍写入性能提升）。redo log 是物理日志（记录"页上改了啥"），undo log 是逻辑日志（记录"SQL 反操作"）——两者相辅相成。
