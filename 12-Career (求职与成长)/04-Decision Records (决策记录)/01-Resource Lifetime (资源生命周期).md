...
> [!summary] 核心摘要
> GC 只回收不可达内存，不替代文件、socket、锁和事务的关闭协议。C++ 用 RAII，Go 用 defer，Python 用 context manager，本质都是把资源生命周期绑定到明确作用域。
...
> [!summary] 核心摘要
> 内存生命周期和外部资源生命周期不是一回事。GC 可以回收不可达内存，却不能替你决定何时提交事务、解锁或关闭 socket；因此资源责任必须在获得处明确。
>
