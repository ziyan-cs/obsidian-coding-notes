...
> [!summary] 核心摘要
> handler 负责协议解析与响应映射，service 维护业务不变量，repository 隔离外部存储。请求链路还必须具有超时、取消、幂等、错误语义和可观测性。
...
> [!summary] 核心摘要
> HTTP handler 做协议转换，service 承载业务规则，repository 封装外部存储。每一层都应能单独测试；timeout、取消和可观测性从请求入口贯穿到依赖调用。
>
