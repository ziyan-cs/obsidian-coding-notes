...
1. 以《TCP/IP 网络编程》为主线梳理系统调用语义，结合个人笔记手动实现核心逻辑
2. CMakeLists.txt 参考成熟工程示例，按多模块可扩展结构规划，为并发模型、日志模块等后续演进预留接口。
3. 参照后端服务器通用设计范式，以 Config/Server 分层封装，便于后续修改。
...
刻意模块化与职责分离，为后续维护和扩展留出空间

- `Config` 类 — 服务器参数的单一来源
集中管理端口、IP、backlog、缓冲区大小，支持默认初始化与自定义覆盖，消除魔法数字散落各处的问题。

- `Server` 类 一 Socket 生命周期的完整封装
底层接口（`creatSocket`、`setSocketOpt`、`bindSocket`、`listenSocket`）与整合接口（`init`、`run`）分层设计，职责边界清晰，便于定位问题逐层排查。

- `main.cpp` 一 只调用初始化与启动接口，不含任何业务代码。
...
>延伸阅读 → [02e-粘包问题与解决](05-Network%20Programming（网络编程）/01%20·%20网络基础/02-TCP%20Deep%20Dive%20(TCP深入)%20⭐/02e-Sticky%20Packet%20Problem%20&%20Solutions%20(粘包问题与解决).md)
...
>此为 TCP 字节流无消息边界的直接体现，详见 → [tcp-byte-stream](09-My%20Projects%20(项目实战)/webserver-cpp/02-insights%20(知识)/tcp-byte-stream.md)
...
虽然来一个连接开一个线程符合直觉，但线程创建与销毁的开销随并发量线性增长，连接一多就会变成资源消耗问题，跳过这一过渡阶段，直接演进 I/O 多路复用更有工程价值。
