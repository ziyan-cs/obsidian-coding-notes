...
这是项目的奠基版本，目标是把 TCP Socket 理论落地，同时搭好后续扩展的基础框架。

CMake 多模块构建参考了示例，根据多模块分步实现功能，
对于项目整体框架有清晰认知，第一步实现

我以《TCP/IP 网络编程》为核心参考，结合资料梳理框架后独立实现

当前目标是实现 server 模块的功能，实现 main 调用，echo 回声功能作为 server 内部附加功能
...
同一时间只能处理一个客户端，新连接必须排队等待。

多线程阻塞 I/O 的处理办法虽然在

### v0.2 目标：

引入 `epoll` ，实现 I/O 多路复用，从 “阻塞单线程” 升级为 “事件驱动并发模型”