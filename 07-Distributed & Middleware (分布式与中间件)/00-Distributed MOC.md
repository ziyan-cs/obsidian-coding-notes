...
- 消息队列：[[07-Distributed & Middleware (分布式与中间件)/03 · Message Queue (消息队列)/03a-Why MQ：Decoupling, Peak Shaving, Async (消息队列三大作用)|MQ 三作用]] · [[07-Distributed & Middleware (分布式与中间件)/03 · Message Queue (消息队列)/03b-Kafka Architecture Overview (Kafka架构概览)/03b1-Topic, Partition, Consumer Group (核心概念)|Kafka]] · [[07-Distributed & Middleware (分布式与中间件)/03 · Message Queue (消息队列)/03c-RabbitMQ vs Kafka：Comparison (两者对比)|RabbitMQ vs Kafka]]
...
## 与 C++ / 项目入口
- 缓存落点：[[06-Database (数据库)/04 · 高可用与架构/12-MySQL vs Redis：Caching Strategy (缓存策略对比)|MySQL vs Redis]]
