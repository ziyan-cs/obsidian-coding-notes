...
****
> [!note] 本节重点：核心考点：C++ 基本类型的大小与范围、类型转换规则、const 与引用
...
> [!note] 本节重点：核心考点：条件分支（if/switch）、循环（for/while/do-while）、break/continue 控制流
...
> [!note] 本节重点：核心考点：函数重载、默认参数、值传递/引用传递、inline 函数
