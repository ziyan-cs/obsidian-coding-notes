...
> [!summary]- 复述检查：学完后再展开
>
> **核心结论**：值类别决定表达式能否被修改、绑定或移动；移动并不移动数据本身，而是允许资源所有权转移，移动后对象仍须保持可析构、可赋值的有效状态。
...
> [!note] 本节重点：移动构造函数与移动赋值运算符的实现、noexcept 的重要性、资源窃取语义

移动语义允许类型把资源所有权从源对象转移到目标对象；对持有独立指针或句柄的类型，这通常避免深拷贝，但移动是否存在、是否更快以及复杂度都由具体类型决定。
