...
```
资源分配图（RAG）：圆形 = 进程，矩形 = 资源
有向边：进程→资源（申请），资源→进程（已分配）
...
互斥锁与信号量详见 → [Mutex（互斥锁）](/01-CS%20Core%20(计算机核心基础)/03-Operating%20System（操作系统）/05-Synchronization（同步与互斥⭐）/01-Mutex（互斥锁）.md) · [Semaphore（信号量）](/01-CS%20Core%20(计算机核心基础)/03-Operating%20System（操作系统）/05-Synchronization（同步与互斥⭐）/02-Semaphore（信号量）.md)
