...
> **核心考点**：OS 四大功能、内核态 vs 用户态、系统调用、操作系统类型与架构
...
// │    Syscall接口（syscall）         │
// │  ┌────┬────┬────┬────┬────┐      │
// │  │Process│Memory│文件│I/O│Network│      │ ← 全在Kernel Space
// │  └────┴────┴────┴────┴────┘      │
// └──────────────────────────────────┘
```
...
> **工程要点**：系统调用是用户态与内核态的唯一入口，每次 syscall 涉及特权级切换（ring 3 → ring 0），是性能关键路径。批量系统调用（如 readv/writev）比多次单调用明显更快。
