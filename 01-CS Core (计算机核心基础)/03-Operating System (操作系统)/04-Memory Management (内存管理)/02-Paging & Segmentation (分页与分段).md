...
# Paging & Segmentation — 分页与分段

> [!important] **核心考点**：分页 vs 分段、多级页表、页表项格式、段页式结合、内存碎片
...
虚拟内存机制详见 → [Virtual Memory（虚拟内存）](/01-CS%20Core%20(计算机核心基础)/03-Operating%20System%20(操作系统)/04-Memory%20Management%20(内存管理)/01-Virtual%20Memory%20(虚拟内存).md)
