...
```
更新数据库 → 失效缓存 → 后续读取回源并回填（Cache-Aside 常用流程）
若缓存删除失败、读写交错或回填使用落后副本，旧值可能持续存在；必须设计重试、TTL/版本号或对账等收敛机制。
...
先定义该字段允许多长时间读到旧值，以及错读的业务损失。
缓存不一致没有通用“几毫秒到几秒”上界；故障、消息丢失或长期 TTL 都可能扩大窗口。
若业务要求写后读一致，优先从权威源读取或使用可验证的版本/会话语义，而非仅凭删除缓存。
```
