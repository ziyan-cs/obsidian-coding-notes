...
> [!note] 本节重点：InnoDB 页结构（数据页/索引页/undo 页）、Buffer Pool 缓存机制与 LRU 管理
...
> [!note] 本节重点： B+ 树索引结构（非叶节点存储键+指针、叶节点存储记录+双向链表）、高度与 IO 次数
...
> [!note] 本节重点：聚簇索引（主键索引即数据）与二级索引的结构差异、回表查询与覆盖索引
...
> [!note] 本节重点：索引条件下推 ICP 减少回表、覆盖索引避免回表、索引合并优化
...
> [!info]- 延伸阅读
> - 下一步：[05-Transactions and Isolation (事务与隔离级别)](/06-Data%20and%20Distributed%20(数据与分布式)/01-MySQL%20and%20SQL%20(MySQL%20与%20SQL)/05-Transactions%20and%20Isolation%20(事务与隔离级别).md)
