...
flowchart TD
    ...
    subgraph "第一层：前端限流"
        Browser -- "秒杀按钮" --> Nginx["Nginx + Lua (OpenResty)"]
        Nginx -- "1. 限流" --> RateLimit["Nginx限流模块<br/>漏桶算法<br/>单IP: 10r/s"]
        Nginx -- "2. 静态页" --> CDN
    end

    subgraph "第二层：业务网关"
        RateLimit --> Gateway["API Gateway"]
        Gateway --> Auth["3. 身份校验 & Token"]
        Gateway --> AntiBot["4. 风控/Bot检测<br/>行为识别 + 设备指纹"]
        Gateway --> Filter["5. 过滤器<br/>限购检查 + 活动时段"]
    end

    subgraph "第三层：库存扣减（核心瓶颈）"
        Filter --> Redis_Stock["Redis Cluster<br/>Lua脚本原子扣减库存"]
        Redis_Stock -- "扣减成功" --> Queue["6. 异步下单<br/>(写入消息队列)"]
        Redis_Stock -- "扣减失败" --> Fail["返回'售罄'"]
    end

    subgraph "第四层：异步下单"
        Queue --> MQ["Kafka / RocketMQ<br/>(顺序写入，削峰)"]
        MQ --> Consumer["Order Consumer<br/>(单线程消费)"]
        Consumer --> DB_Check["MySQL<br/>幂等校验 + 限购二次检查"]
        DB_Check --> Order_Create["生成订单 (INSERT)"]
        Order_Create --> Cache_Update["更新 Redis 库存状态"]
        Order_Create --> Notify["异步通知用户结果"]
    end

    subgraph "监控 & 兜底"
        Prometheus["Prometheus + Grafana<br/>全链路监控"] --> Alert["告警"]
        Sentinel["Sentinel / Hystrix<br/>熔断降级"] --> Fallback["降级策略<br/>直接返回'排队中'"]
    end
```
...
- [Cache Penetration / Breakdown / Avalanche](08-Distributed%20&%20Middleware%20(分布式与中间件)/01%20·%20Redis%20(缓存与数据结构)/01d-Cache%20Patterns%20&%20Problems%20(缓存三大问题)%20⭐.md)
- [MySQL vs Redis Caching Strategy](06-Database%20(MySQL)/04%20·%20高可用与架构/12-MySQL%20vs%20Redis：Caching%20Strategy%20(缓存策略对比).md)
- [Kafka Message Delivery Guarantees](08-Distributed%20&%20Middleware%20(分布式与中间件)/03%20·%20Message%20Queue%20(消息队列)/03b-Kafka%20Architecture%20Overview%20(Kafka架构概览)/03b2-Message%20Delivery%20Guarantees%20(消息可靠性).md)
- [Distributed Transaction 2PC & Saga](08-Distributed%20&%20Middleware%20(分布式与中间件)/04%20·%20Distributed%20Protocols%20(分布式协议)/04e-Distributed%20Transaction：2PC%20&%20Saga%20(分布式事务).md)
- [Rate Limiting & Circuit Breaker](08-Distributed%20&%20Middleware%20(分布式与中间件)/02%20·%20Nginx%20(反向代理与负载均衡)/02b-Reverse%20Proxy%20&%20Load%20Balancing%20Config%20(反向代理配置).md)
