...
> [!summary]- 复述检查：学完后再展开
> **回答**：面试题不是孤立答案库。回答应先给结论，再解释对象模型、系统调用或数据结构等机制，补充边界与工程取舍，并能落到本库对应专题和真实代码。
>
> ## 说明
>
...
50. **select / poll / epoll 对比**
    在 glibc/Linux 中，select 的 `fd_set` 固定为 1024 位，每次调用都传入并改写集合；poll 传入 `pollfd` 数组，没有该固定集合限制，但仍需检查数组。epoll 把 interest list 保留在内核，并从 ready list 返回至多 `maxevents` 个事件，避免每次等待都扫描全部关注项；返回成本与就绪结果数等因素有关，不能简单表述为 O(1)。红黑树与就绪链表属于 Linux 内核实现细节。
    **追问方向：** epoll 的 ET 模式为什么性能更好？使用时需要注意什么？

51. **epoll ET vs LT 模式**
    LT（水平触发）是默认模式：条件持续满足时仍会报告事件。ET（边缘触发）只在状态变化时报告，应用通常把 fd 设为非阻塞，并持续读写直到 `EAGAIN`，否则可能因未排空数据而长期收不到新边沿，或被阻塞调用卡住事件循环。ET 可能减少重复通知，但不保证在所有负载下更快。
    **追问方向：** 为什么 ET 模式必须配合非阻塞 IO 使用？

52. **Reactor 和 Proactor 的区别**
    Reactor 通知“可以执行 I/O”，应用随后完成读写；Proactor 提交异步操作，在操作完成后接收结果。Windows IOCP 常用于完成通知模型；Linux 上传统 epoll 属于就绪通知，`io_uring` 可以提供更接近完成驱动的异步接口。Boost.Asio 对不同平台能力做统一抽象，不能简单归为 `libaio` 模拟。
    **追问方向：** Proactor 模式为什么在 Linux 上需要模拟？Boost.Asio 的实现思路？

53. **HTTPS TLS 握手过程**
    TLS 1.2 与 TLS 1.3 的握手不能混成同一固定流程。共同主线是协商版本与参数、验证服务端证书、通过密钥交换导出会话密钥，再用对称 AEAD 保护应用数据。TLS 1.3 移除了静态 RSA 密钥交换并通常以 (EC)DHE 提供前向安全；首次握手通常 1-RTT，恢复连接才可能使用具有重放风险的 0-RTT 数据。
...
57. **零拷贝：mmap + sendfile**
    “零拷贝”通常指减少 CPU 参与的数据复制和用户态/内核态往返，并不保证物理上零次复制。`mmap` 把文件页映射到进程地址空间，适合应用需要访问内容的场景；`sendfile` 可在内核中把文件数据送往 socket，避免先 `read` 到用户缓冲区。实际数据路径仍取决于内核、文件系统、设备和协议栈。
