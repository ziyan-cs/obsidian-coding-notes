...
> [!note] 本节重点：核心考点：> 存储金字塔结构、Cache 的位置与作用、局部性原理的硬件利用、不同层级的速度差异
...
> [!note] 本节重点：核心考点：> Cache 映射方式（直接/组相联/全相联）、替换策略（LRU/LFU/FIFO）、写策略（写直达/写回）
...
> [!note] 本节重点：核心考点：字节序（大小端）、数据对齐、存储层次、数据持久化
...
> [!note] 本节重点：核心考点：总线结构（数据/地址/控制）、总线仲裁、总线事务、常见总线标准
...
> [!note] 本节重点：核心考点：I/O 三种方式（程序控制/中断/DMA）、中断处理流程、DMA 传输机制
