...
> [!note] 本节重点：核心考点：虚拟地址 vs 物理地址、MMU/TLB、缺页中断、页面置换、页表结构
...
> [!note] 本节重点：核心考点：分页 vs 分段、多级页表、页表项格式、段页式结合、内存碎片
...
> [!note] 本节重点：核心考点：文件系统结构、inode、目录结构、文件分配方式、磁盘调度、硬链接 vs 软链接
...
- 不看正文，用“问题 → 机制 → 边界”三句话讲清 **06-Virtual Memory and File System (虚拟内存与文件系统)**。
