...
> [!note] 本节重点：CPU 核心组成（数据通路 + 控制单元）、寄存器、ALU、控制器实现方式
...
> [!note] 本节重点：取指-译码-执行周期、CPU 流水线各阶段、流水线冒险与解决
...
> [!note] 本节重点：指令执行全流程、各阶段微操作、流水线深度对性能的影响
...
> [!note] 本节重点：编译四阶段（预处理/编译/汇编/链接）、静态链接 vs 动态链接、链接器符号解析与重定位
