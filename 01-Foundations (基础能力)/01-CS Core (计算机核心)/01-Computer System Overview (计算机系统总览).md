...
> [!summary]- 复述检查：学完后再展开
>
> **核心结论**：计算机遵循存储程序模型：指令和数据以二进制存入内存，CPU 按取指、译码、执行的循环运行；输入、输出、运算、控制与存储共同完成计算。

# Computer System Overview (计算机系统总览)

> [!note] 本节重点：计算机的五大逻辑部件、冯·诺依曼架构、存储程序概念、指令与数据同存
...
> [!note] 本节重点：计算机系统的层次抽象模型、每层屏蔽下层细节、抽象是控制复杂度的核心手段
...
> [!note] 本节重点：软硬件的逻辑等价性、分层抽象、固件的角色、ISA 作为软硬件分界
