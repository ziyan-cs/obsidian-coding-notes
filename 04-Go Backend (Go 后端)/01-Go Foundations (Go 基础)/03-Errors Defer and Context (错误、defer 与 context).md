...
# 03-Errors Defer and Context (错误、defer 与 context)

> [!abstract] 一句话结论：Go 用显式 `error` 表达可预期失败，用 `defer` 绑定局部清理，用 `context` 传播请求取消、截止时间和请求范围数据。

## 30 秒回答

**03-Errors Defer and Context (错误、defer 与 context)**：先说明它解决的问题，再解释一个关键机制、一个边界条件，并用最小示例或真实项目验证。
...
- 不看正文，说明 03-Errors Defer and Context (错误、defer 与 context) 的问题、核心机制与边界。
