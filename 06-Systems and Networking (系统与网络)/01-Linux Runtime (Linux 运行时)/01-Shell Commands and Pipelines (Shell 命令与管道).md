...
> [!tip]- **工程要点**：
> - 可将 `set -euo pipefail` 作为防护网，但关键步骤仍需显式检查与错误信息
...
Shell 的难点不是记住命令，而是理解一行文本如何变成进程、参数和数据流。可以按以下顺序阅读一条命令：

```text
source text
   -> parsing / expansion / quoting
   -> argv + redirections
   -> processes connected by pipes
   -> exit status observed by the parent shell
```

- **引用（quoting）**决定空格、通配符和 `$` 是否仍具有特殊含义。
- **展开（expansion）**可能把一个词变成零个、一个或多个参数。
- 管道 `a | b` 传递的是字节流；它不会自动传递结构化对象，也不会让 `b` 知道 `a` 的记录边界。
- 退出状态是自动化的控制接口：通常 `0` 表示成功，非 `0` 表示某类失败，但具体含义应查命令文档。

```bash
name='report 2026.txt'
printf '%s\n' "$name"       # 一个参数
printf '%s\n' $name         # 被词拆分为两个参数；通常不是想要的行为
```
...
文件名可以包含空格、换行和前导 `-`。因此不要用 `for f in $(find ...)` 处理未知文件集合。优先使用工具原生的 `-exec`，或 NUL 分隔协议：
...
将用户输入作为参数传递，不要拼接后交给 `eval`；删除、覆盖和批量移动前先输出将要操作的目标，并使用测试目录做演练。

### `set -euo pipefail` 不是错误处理器

它适合许多非交互脚本作为防护网，但不能替代显式处理：`set -e` 在条件测试、逻辑连接、子 Shell 与不同 Shell 实现中有上下文规则；`pipefail` 会改变管道整体状态；`-u` 也会影响可选变量。关键步骤应主动检查并给出可诊断信息：

```bash
if ! output=$(important_command 2>&1); then
    printf 'important_command failed: %s\n' "$output" >&2
    exit 1
fi
```

跨机器脚本先确定解释器与兼容范围：需要 Bash 特性就写 `#!/usr/bin/env bash`，不要声称是 POSIX `sh`。

## 调试与质量检查

- `bash -n script.sh`：只做语法检查。
- `bash -x script.sh`：打印展开后的执行轨迹；注意输出可能含密钥。
- `shellcheck script.sh`：发现未引用变量、不可移植写法和常见陷阱。
- `command -v tool`：验证依赖命令是否存在。
- `trap`：清理临时文件或在退出时记录上下文，但信号处理函数应保持简单。
...
给定一个会批量读取日志、筛选字段并生成报告的脚本，你应能说明每一步的输入输出、正确处理带空格文件名、保留失败退出码，并用 `shellcheck` 与小型样例验证。命令速查只解决“工具叫什么”，执行模型才能解决“为什么脚本在边界条件下出错”。
