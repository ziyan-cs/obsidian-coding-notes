...
| 维度 | 进程 | 同一进程内的线程 |
| --- | --- | --- |
| 虚拟地址空间 | 通常各自独立，可借共享映射通信 | 共享地址空间与大部分进程资源 |
| 私有执行状态 | 有自己的线程集合与地址空间 | 各自有栈、寄存器、线程本地存储 |
| 故障隔离 | 一个进程崩溃通常不直接终止另一进程 | 一个线程触发未处理致命信号可能终止整个进程 |
| 协调成本 | 需明确 IPC、序列化或共享内存同步 | 共享数据方便，但数据竞争与锁顺序成为主要风险 |

`fork()` 复制进程描述状态并通常以写时复制处理内存页；`pthread_create()` 创建同进程的新执行流。两者的耗时随页表规模、运行库、内核、CPU、资源限制和负载变化，不能背“100 μs 对 10 μs”或“协程切换 100 ns”的固定表。协程是否切到另一个 OS 线程以及其调度成本，也取决于语言运行时。

## 上下文切换为何有代价

切换需要保存/恢复执行状态并让调度器选择下一任务；跨地址空间时还要改变内存映射上下文。缓存/TLB 扰动可能增加成本，但现代处理器和内核可用 ASID/PCID 等机制保留部分 TLB 条目，**不是每次进程切换都刷空整个 TLB**。文件描述符表也不是在每次切换时复制；它是任务关联的内核状态。

要比较真实开销，在同一机器上控制线程数、进程地址空间规模、亲和性与负载，再用 `perf stat`、调度跟踪或基准实验测量；不要从语言名直接推导吞吐。
