...
从项目根目录运行 `python -m example_app.cli`，让解释器按模块语义定位入口。直接运行包内部文件会改变 import 上下文，容易出现“脚本能跑，安装后不能跑”的问题。模块间优先使用明确的绝对导入；循环导入通常说明职责或共享类型放错层。
...
1. 创建空目录与独立环境，打印 `sys.executable`。
2. 建立 `src` 包和 `python -m` 入口，确认从非源码工作目录安装后仍能运行。
3. 加入一个运行依赖和一个开发依赖，观察 `pyproject.toml` 与 lockfile 各自变化。
4. 删除环境后仅凭仓库文件重建，运行测试并比较结果。
# 官方资料
