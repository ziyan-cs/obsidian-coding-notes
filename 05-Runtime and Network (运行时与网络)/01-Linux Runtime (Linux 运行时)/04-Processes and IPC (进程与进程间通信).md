...
> [!summary]- 复述检查：学完后再展开
>
> **回答**：进程拥有独立地址空间和内核资源，IPC 用管道、共享内存、消息队列或 socket 跨越隔离边界。选型要同时考虑数据量、同步方式、复制成本和故障隔离。
...
> [!info]- 延伸阅读
> - 下一步：[05-POSIX Thread Lifecycle (POSIX 线程生命周期)](/05-Runtime%20and%20Network%20(运行时与网络)/01-Linux%20Runtime%20(Linux%20运行时)/05-POSIX%20Thread%20Lifecycle%20(POSIX%20线程生命周期).md)
