...
> [!summary]- 复述检查：学完后再展开
>
> **回答**：Linux 文件访问由路径解析、inode 元数据和用户/组/其他权限共同决定；软链接保存路径，硬链接增加同一 inode 的目录项。排查时要同时看身份、每级目录执行权限、环境变量与实际解析路径。
>
> > [!note] 本节重点：Linux 文件系统层次结构、文件权限 rwx 与 chmod/chown、inode 与硬软链接
