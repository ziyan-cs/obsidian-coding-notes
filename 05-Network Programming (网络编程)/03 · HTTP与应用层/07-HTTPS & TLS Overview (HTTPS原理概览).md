...
> **核心考点**：HTTPS 的 TLS/SSL 握手流程、证书验证与数字签名、对称+非对称加密混合方案
...
> **工程要点**：TLS 握手的主要成本是首次的 2-RTT（TLS 1.2）。生产环境通过会话复用（Session ID/Session Ticket）减少握手次数。对于移动端和 API 服务，TLS 1.3 的 0-RTT 能显著降低首字节延迟。注意 0-RTT 有重放攻击风险，需要应用层幂等性保护。
