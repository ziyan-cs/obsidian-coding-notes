...
- 概念
	- **crond**：后台进程，监听并自动触发定时任务
	- **crontab**：命令行工具，用户自己配置定时任务时使用

- **crontab** 核心命令
	- `crontab -e`：新增 / 编辑 / 删除工作用户定时任务
	- `crontab -l`：查看工作用户所有定时任务
	- `crontab -r`：删除工作用户所有定时任务（谨慎使用）
	- `service crontab restart`：重启所有定时任务
	- `crontab -u [user_name] [-e/-l/-r]`：
		- 查看 / 编辑 / 删除另一用户的定时任务（需 root 权限）
...
- 概念
	- **atd**：后台进程，监听并自动触发一次性定时任务
	- **at**：命令行工具，用户自己配置一次性定时任务时使用
...
1. **绝对路径**：命令 / 脚本必须写绝对路径，避免因环境变量缺失导致失效。
2. **脚本权限**：脚本需添加执行权限 (`chmod +x`) 并首行指定解释器 (`#!/bin/bash`)。
3. **日志排查**：任务不执行时，通过 `grep CRON /var/log/syslog` 排查错误。
4. **服务状态**：确保 crond 服务运行 (`systemctl start crond`) 并开机自启 (`systemctl enable crond`)。

## 6.6 总结

- 定时任务核心 = **crond 服务 + crontab 配置**。
- 核心命令：
- 配置关键：**时间格式准确 + 绝对路径 + 权限配置**。
- 适用场景：系统维护、数据备份、自动化脚本执行。
# 7. 磁盘管理
