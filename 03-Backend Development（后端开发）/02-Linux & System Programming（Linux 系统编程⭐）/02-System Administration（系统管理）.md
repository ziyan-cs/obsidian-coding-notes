...
- 设置添加硬盘ffffffffff
	- 在虚拟机设置中添加新硬盘，分配容量并保存
	- 系统重启，新硬盘通常对应设备名 `/dev/sdb`（第一块新硬盘）
...
- 格式化
	- **核心命令**：`mkfs -t ext4 /dev/sdb1`
	    
	    - `ext4`：主流 Linux 文件系统
	    - `/dev/sdb1`：待格式化的分区
	    
	- **作用**：将分区 “初始化” 为系统可识别的存储格式。

#### 4. 挂载（mount）

- **创建挂载点**：`mkdir -p /data`（新建一个目录作为入口）
- **执行挂载**：`mount /dev/sdb1 /data`
- **验证**：`df -h` 查看 `/data` 目录是否已占用硬盘空间。

#### 5. 设置自动挂载（/etc/fstab）

- **原理**：`mount` 挂载仅临时有效，重启失效；修改 `/etc/fstab` 可实现开机自动挂载。
- **操作步骤**：
    
    1. 查看分区 UUID（唯一标识）：`blkid /dev/sdb1`
    2. 编辑配置文件：`vim /etc/fstab`
    3. 添加一行规则：
...
|步骤|核心命令|作用|
|:--|:--|:--|
|**分区**|`fdisk /dev/sdb`|划分硬盘区域|
|**格式化**|`mkfs.ext4 /dev/sdb1`|建立文件系统|
|**挂载**|`mount /dev/sdb1 /data`|临时关联目录|
|**自动挂载**|`vim /etc/fstab` + `mount -a`|永久生效配置|