...
- **任务调度**：系统在**指定时间点 / 周期**自动执行特定命令或程序
- **分类**
    - **系统工作**：全局、周期性、系统级维护（如病毒扫描、日志轮转）。
    - **个别用户工作**：针对特定用户（如 MySQL 数据库备份、脚本定时执行）。

- **核心工具**
	- **crond**：Linux 后台守护进程，持续监听定时任务并自动触发。
	- **crontab**：用户配置定时任务的命令行工具，管理任务清单。

### 6.2 crontab 核心命令

- `crontab -e`：新增 / 修改 crontab 定时任务
- `crontab -l`：查询所有 crontab 定时任务
- `crontab -r`：删除 **所有** 的 crontab 任务（谨慎使用）
- `crontab -u`：指定用户操作（需 root 权限)
...
|---|---|---|
|*****|任意值（匹配所有时间）|`* * * * *` → 每分钟执行一次|
|**,**|分隔多个时间点|`0 8,12 * * *` → 每天 8 点、12 点各执行一次|
|**-**|时间范围|`0 9-18 * * *` → 每天 9 点至 18 点，每小时执行一次|
|**/n**|每隔 n 单位执行|`*/10 * * * *` → 每 10 分钟执行一次|
