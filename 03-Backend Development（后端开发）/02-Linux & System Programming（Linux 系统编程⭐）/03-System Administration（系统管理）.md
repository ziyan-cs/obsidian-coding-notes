...
- GRUB 修改启动参数 → 进入单用户模式 → 绕过登录 → 修改密码

- **说明**
	- 必须读写挂载根目录
	- 仅适用物理机 / 拥有服务器物理控制权的场景
	- 「 *Ubuntu* 默认禁用 `root` 账户，`passwd root` 解锁 `root` 并设置密码 」

- **CentOS 步骤**
	- 1. 开机默认 GRUB 菜单，按 `e` 编辑
	- 2. `linux16` 行末尾加 `init=/bin/sh`，`Ctrl+X` 进入单用户模式
	- 3. 输 `mount -o remount,rw /` 读写挂载根目录
	- 4. 输 `passwd`
	- 5. 依次 输两次新密码
	- 6. 输 `touch /.autorelabel`
	- 7. 输 `exec /sbin/init`
	- 8. 等待
	- 9. 系统自动重启，新密码生效
...
- **Ubuntu 步骤**
	...
	- 2. `linux` 行末尾 `ro` 改为 `rw init=/bin/bash`，`Ctrl+X` 进入单用户模式
	- 3. 输入 `passwd root`
	- 4. 依次 输入两次新密码
	- 5. 输入 `reboot -f` 
	- 6. 系统强制重启，新密码生效
